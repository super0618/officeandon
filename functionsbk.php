    ...
    foreach ($tables as $table) {
            ...
            if($table != 'Period_Cumulative' && $table != 'SHIFT_PATTERN') {
                ...
                foreach($g_items as $item){
                    ...
                    if($value_name == 'shift_plan' || $value_name == 'ot_plan') {
                        ...
                        $data[$table][$machine][$value_name]['value'] = get_shift_plan($plan_date, $plan_shift, $machine, $value_name);
                    }
                    ...
                    if( $value_name == 'actual' ||
                        ...
                        if(isset($settings[$tag_field]) && !empty($settings[$tag_field])) {
                            if($value_name == 'shift_line_stop') {
                                $line_stop_tags = explode(",", $settings[$tag_field]);
                                foreach ($line_stop_tags as $line_stop_tag)
                ...
                if(count($tags) > 0) {
                    ...
                    while($row=mysqli_fetch_object($result)) {
                        ...
                        if(isset($line_stop_tags) && !empty($line_stop_tags) && in_array($row->name, $line_stop_tags) ) {
                ...
                if($target_stopped == false) {
                    ...
                    $value = get_target($opr, $start_time, $target_time, $shift_info);
                ...
                if($table != "LOW_PRESSURE_CASTING" && $table != "HIGH_PRESSURE_CASTING") {
                    ...
                    $last_hour_target = get_target($opr, $last_start_time, $last_end, $shift_info);
        ...
        if($table == "LOW_PRESSURE_CASTING" || $table == "HIGH_PRESSURE_CASTING") {
            ...
            foreach ($EDMs as $EDM){
                ...
                if(isset($settings['actual_tag']) && !empty($settings['actual_tag'])) {
                    /*if($EDM == 'H1' || $EDM == 'H2')
                        $value = get_actual_value($settings['actual_tag'], $start_time, $last_start_time);
                    else
...
function read_history_values($post_data)
{
    ...
    if($start_time > $current) {
        $data['error'] = 'Select old date and shift';
    } elseif($start_time <= $current && $end_time >= $current) {
        $data = get_main_data($page);
    } else
...
function get_target($opr, $start, $end, $shift_info)
{
    global $current;
    if($end > $current)
        $end = $current;
    ...
    if($check == 0) {
        if($plan_name == 'shift_plan')
            $sql = "INSERT INTO {$tblShiftPlan} (`plan_date`,`shift_plan`, `ot_plan`, `opr`, `shift`, `machine`) 
                            VALUES ('{$plan_date}', '{$plan}', 0, 0, '{$shift}', '{$machine}')";
        else if($plan_name == 'ot_plan')
            $sql = "INSERT INTO {$tblShiftPlan} (`plan_date`,`shift_plan`, `ot_plan`, `opr`, `shift`, `machine`) 
                            VALUES ('{$plan_date}', 0, '{$plan}', 0, '{$shift}', '{$machine}')";
        else
            $sql = "INSERT INTO {$tblShiftPlan} (`plan_date`,`shift_plan`, `ot_plan`, `opr`, `shift`, `machine`) 
                            VALUES ('{$plan_date}', 0, 0, '{$plan}', '{$shift}', '{$machine}')";

        $res = $db->query($sql);
    } else {
        if($plan_name == 'shift_plan')
            $sql = "UPDATE {$tblShiftPlan} SET `shift_plan` = '{$plan}' WHERE `plan_date` ='{$plan_date}' AND `machine` = '{$machine}' AND `shift` = '{$shift}'";
        else if($plan_name == 'ot_plan')
            $sql = "UPDATE {$tblShiftPlan} SET `ot_plan` = '{$plan}' WHERE `plan_date` ='{$plan_date}' AND `machine` = '{$machine}' AND `shift` = '{$shift}'";
        else
            $sql = "UPDATE {$tblShiftPlan} SET `opr` = '{$plan}' WHERE `plan_date` ='{$plan_date}' AND `machine` = '{$machine}' AND `shift` = '{$shift}'";
